...
USBStates USBState = Disconnected;

unsigned long lastPoll = 100; // this is the millis() of the last time we polled the usb
...
void pollUSB()
{
  ...
  case Disconnected:
    ...
    else if (currentUSB->getUsbTaskState() == USB_STATE_RUNNING)
    {
      ...
      {
        USBState = XBOXONEConnected;
      }
      else if (PS3.PS3Connected)
      {
        USBState = PS3Connected;
      }
...
void setup()
{
  ...
  if (keebH.Init())
  {
    while (true) //halt if keebH.Init() fails
    {
      digitalWrite(LEDPin, LOW);
      delay(100);
      digitalWrite(LEDPin, HIGH);
